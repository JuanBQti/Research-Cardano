#!/usr/bin/env python3
"""Delegator reward per unit heatmaps: a0=0.3 vs a0=0.6 (+ difference)."""
...
FONT_SIZE = 12
OUTPUT_PATH = OUT_DIR / "heatmap_delegator_reward_a0_cases.png"
...
def main() -> None:
    S, P = sigma_p_grids()
    vals_base = delegator_grid(S, P, z0, r_over_t, a0, c_i, m_i)
    vals_new = delegator_grid(S, P, z0, r_over_t, a0_alt, c_i, m_i)
    vals_diff = vals_new - vals_base
    ...
    draw_panel(
        ...
        title=rf"Difference: $a_0={a0_alt}$ minus $a_0={a0}$",
        cbar_label=r"$\Delta$ reward / unit stake",
        diverging=True,
    )
    fig.suptitle(
        "Delegator rewards per unit when $a_0$ changes\n"
