#!/usr/bin/env python3
"""Reward function f(sigma, p; z0) vs pool stake: k=500 vs k=1000."""
...
FONT_SIZE = 12
OUTPUT_PATH = OUT_DIR / "Reward_function_vs_sigma.png"
...
def main() -> None:
    ...
    y_k1000 = gross_pool_reward(sigma, p_i, z0_alt, r_over_t, a0)
    feasible = sigma >= p_i
    y_base = np.where(feasible, y_base, np.nan)
    y_k1000 = np.where(feasible, y_k1000, np.nan)

    fig, ax = plt.subplots(1, 1, figsize=(9, 5), constrained_layout=True)
    ax.plot(sigma, y_base, linewidth=2.2, label=rf"$k={k}$, $z_0={z0/1e6:.1f}$M")
    ax.plot(
        sigma,
        y_k1000,
        linewidth=2.2,
    ...
    ax.set_title(
        r"Reward function $f(\sigma_i,p_i;z_0)$ vs pool size"
        "\n"
        rf"$a_0={a0}$, $p_i={p_i/1e3:.0f}$K; $k={k}$ vs $k={k_alt}$",
        fontsize=FONT_SIZE,
    )
