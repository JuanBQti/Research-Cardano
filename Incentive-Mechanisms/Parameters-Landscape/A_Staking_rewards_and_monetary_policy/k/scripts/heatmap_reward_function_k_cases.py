#!/usr/bin/env python3
"""Gross pool reward f(sigma, p; z0) heatmaps: k=500 vs k=1000 (+ difference)."""
...
FONT_SIZE = 12
OUTPUT_PATH = OUT_DIR / "heatmap_reward_function_k_cases.png"
...
def main() -> None:
    S, P = sigma_p_grids()
    vals_500 = gross_grid(S, P, z0, r_over_t, a0)
    vals_1000 = gross_grid(S, P, z0_alt, r_over_t, a0)
    ...
    draw_panel(
        axes[2], S, P, vals_diff, z0_alt,
        title=rf"Difference: $k={k_alt}$ minus $k={k}$",
        cbar_label=r"$\Delta f$ (ADA)",
        diverging=True,
    )
    fig.suptitle(
        rf"Reward function $f(\cdot)$ when $k$ changes" "\n" rf"$a_0={a0}$",
        fontsize=FONT_SIZE,
    )
