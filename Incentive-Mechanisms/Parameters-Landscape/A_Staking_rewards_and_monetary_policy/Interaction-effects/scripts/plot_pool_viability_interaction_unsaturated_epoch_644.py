...
Same 2x2 as plot_pool_viability_interaction_effects_epoch_644.py, but only
pledge-met pools that remain unsaturated if k rises to 1000 (sigma <= T/1000).
"""
...
OUT_PNG = DIR / "pool_viability_interaction_unsaturated_epoch_644.png"
OUT_CSV = DIR / "pool_viability_interaction_unsaturated_epoch_644.csv"
...
def main() -> None:
    ...
    unsaturated_k1000 = sigma_a <= z0_1000
    eligible = pledge_met & unsaturated_k1000
    n_excluded_oversat = int((pledge_met & ~unsaturated_k1000).sum())
    ...
    h0, n0, v0, l0, r0, cat0, f0, pi0 = compute_viability(
        sigma_a, declared_a, active_a, cost_a, margin_a, eligible, z0_500, r_over_t, a0
    )
    hA, nA, vA, lA, rA, catA, fA, piA = compute_viability(
        sigma_a, declared_a, active_a, cost_a, margin_a, eligible, z0_1000, r_over_t, a0
    )
    hB, nB, vB, lB, rB, catB, fB, piB = compute_viability(
        sigma_a, declared_a, active_a, cost_170, margin_a, eligible, z0_1000, r_over_t, a0
    )
    hC, nC, vC, lC, rC, catC, fC, piC = compute_viability(
        sigma_a, declared_a, active_a, cost_75, margin_a, eligible, z0_500, r_over_t, a0
    )
    hD, nD, vD, lD, rD, catD, fD, piD = compute_viability(
        sigma_a, declared_a, active_a, cost_75, margin_a, eligible, z0_1000, r_over_t, a0
    )
    ...
    fig.suptitle(
        r"Epoch 644 — theoretical viability (unsaturated if $k\to1000$)"
        "\n"
        + rf"$r=\Pi_i/C^*$, $C^*={C_STAR_ADA:.1f}$ ADA/epoch; "
        + rf"$R={R/1e6:.2f}$M ADA, $T={T/1e9:.2f}$B, $a_0={a0}$; "
        + rf"included: {n0} ($\sigma\leq z_0(1000)$); excluded oversaturated: {n_excluded_oversat}",
        fontsize=FONT_SIZE,
    )
    ...
    print(f"Included unsaturated pledge-met: {n0}; excluded oversaturated: {n_excluded_oversat}")
