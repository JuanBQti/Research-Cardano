#!/usr/bin/env python3
"""Heatmaps of theoretical f and Pi when (a0, c) move from (0.3, 170) to (0.6, 75).

R=14.9M, T=38.8B, k=500, m=5%. Each figure: baseline | counterfactual | % difference.
Note: f depends on a0 (not c); Pi uses both a0 and the pool fixed cost c.
"""
...
OUT_F = DIR / "heatmap_f_a0_0p6_c75_interaction.png"
OUT_PI = DIR / "heatmap_Pi_a0_0p6_c75_interaction.png"
...
def main() -> None:
    ...
    f0 = gross(S, P, z0, r_over_t, A0_0)
    f1 = gross(S, P, z0, r_over_t, A0_1)
    pi0 = operator(S, P, z0, r_over_t, A0_0, C0, M)
    pi1 = operator(S, P, z0, r_over_t, A0_1, C1, M)
    ...
    draw_abs(
        axes[0], S, P, f0, z0,
        title=rf"$k={K}$, $a_0={A0_0}$",
        cbar_label=r"$f(\sigma_i,p_i)$ (ADA)",
        vmax=vmax_f,
    )
    draw_abs(
        axes[1], S, P, f1, z0,
        title=rf"$k={K}$, $a_0={A0_1}$",
        cbar_label=r"$f(\sigma_i,p_i)$ (ADA)",
        vmax=vmax_f,
    )
    draw_pct(
    ...
    fig.suptitle(
        r"Gross pool reward $f$: $(a_0,c)=(0.3,170)\to(0.6,75)$"
        "\n"
        + rf"$R={R/1e6:.1f}$M, $T={T/1e9:.1f}$B, $k={K}$"
        + r" ($f$ depends on $a_0$, not $c$)",
        fontsize=FONT_SIZE,
    )
    ...
    draw_abs(
        axes[0], S, P, pi0, z0,
        title=rf"$a_0={A0_0}$, $c={C0:.0f}$",
        cbar_label=r"$\Pi_i$ (ADA)",
        vmax=vmax_pi,
    )
    draw_abs(
        axes[1], S, P, pi1, z0,
        title=rf"$a_0={A0_1}$, $c={C1:.0f}$",
        cbar_label=r"$\Pi_i$ (ADA)",
        vmax=vmax_pi,
    )
    draw_pct(
        axes[2], S, P, pct_pi, z0,
        title=r"% difference: final minus initial",
    )
    fig.suptitle(
        r"Operator reward $\Pi$: $(a_0,c)=(0.3,170)\to(0.6,75)$"
        "\n"
        + rf"$R={R/1e6:.1f}$M, $T={T/1e9:.1f}$B, $k={K}$, $m={100*M:.0f}\%$",
        fontsize=FONT_SIZE,
    )
