...
From the operator perspective, the vast majority of pools already operate at a deficit under baseline conditions, failing to cover basic operational expenses ($C^*$). Increasing the pledge factor further concentrates operator rewards into a small subset of heavily pledged pools, driving more marginal pools into unprofitability. When low fixed fees ($c_i = 170$ or $c_i = 75$) are enforced, operator margins contract sharply across the board. Therefore, a lower declared fixed costs combined with higher pledge ($a_0$) severely degrades the economic viability of smaller and mid-sized stake pool operators, making long-term sustainability attainable only for elite, high-pledge, and heavily delegated operations. Notice, however, that, for our exercise, we imposed that all pools choose $c_i=`minPoolCost`. Those small pools that heavily relies on the declared fixed cost for their revenues will not naturally choose to reduce it. However, as we will see in the next subsection, this decision may affect their delegators APR and hence their attractivness.
