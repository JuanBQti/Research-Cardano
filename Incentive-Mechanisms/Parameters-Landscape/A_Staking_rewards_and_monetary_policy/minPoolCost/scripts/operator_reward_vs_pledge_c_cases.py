#!/usr/bin/env python3
"""Operator reward vs pledge when reported min pool cost c changes (k=500)."""
...
FONT_SIZE = 12
OUTPUT_PATH = OUT_DIR / "operator_reward_vs_pledge_c_cases.png"
...
def operator_reward(
    ...
    p_arr = np.asarray(p, dtype=float)
    f_val = gross_pool_reward(sigma, p_arr, z0_ada, r_scale, a0_value)
    share = m + (1.0 - m) * (p_arr / sigma)
...
def main() -> None:
    ...
    for sigma_val, c_val, style in cases:
        y = operator_reward(sigma_val, p, z0, r_over_t, a0, c_val, m_i)
    ...
    ax.set_title(
        "Operator reward vs pledge (k=500)\n"
