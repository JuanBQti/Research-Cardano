#!/usr/bin/env python3
"""
Bubble chart: pool counts by declared fixed cost and stake size.
All pools (stake ≥ 1 ADA). Epoch 644.

Major fixed-cost values are shown separately; sparse values are
grouped as "other".

Usage:
  python3 plot_fixed_cost_170_vs_340_stake_bubbles_epoch_644.py
"""
...
OUT_PLOT = DIR / "fixed_cost_170_vs_340_stake_bubbles_epoch_644.png"

FONT_SIZE = 12
MIN_STAKE = 1.0  # ADA
# Show these costs as their own columns; everything else → "other"
    ...
    out = pd.DataFrame(
        {
            ...
            / 1e6,
            "stake_ada": pd.to_numeric(df["epochs.0.data.epoch_stake"], errors="coerce")
            / 1e6,
            "margin": pd.to_numeric(df["pool_update.active.margin"], errors="coerce"),
        }
    ).dropna(subset=["cost_ada", "stake_ada", "margin"])
    out = out[out["stake_ada"] >= MIN_STAKE].copy()
...
def main() -> None:
    ...
    rows: list[dict] = []
    for g in groups:
        stake = data.loc[data["group"] == g, "stake_ada"].to_numpy()
        counts, _ = np.histogram(stake, bins=EDGES)
    ...
    xticklabels = [f"{int(c)}" if isinstance(c, float) else c for c in groups]
    # annotate column totals in tick labels
    totals = {g: int((data["group"] == g).sum()) for g in groups}
    xticklabels = [
        f"{lab}\n(n={totals[g]})" for lab, g in zip(xticklabels, groups)
    ]
    ...
    ax.set_ylabel("Epoch stake (ADA bins)", fontsize=FONT_SIZE)
    ax.set_title(
        "Epoch 644 — pool counts by fixed cost and stake size\n"
    ...
    ax.text(
        0.02,
        0.02,
        f"n={len(data)} pools with stake ≥ {MIN_STAKE:g} ADA; "
        f"other covers {len(other_vals)} distinct costs",
